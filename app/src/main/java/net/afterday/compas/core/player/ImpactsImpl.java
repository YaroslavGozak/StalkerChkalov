...
public class ImpactsImpl implements Impacts
{
    ...
    private static final int BURER_COOLDOWN = 5 * 1000; //Бюрер
    private static final int CONTROLLER_COOLDOWN = 3 * 1000; //Контролёр
    private static final int MENTAL_COOLDOWN = 5 * 1000; //Ментальное излучение
    private static final int ANOMALY_COOLDOWN = 5 * 1000; //Аномалия
    private static final int MONOLITH_COOLDOWN = 5 * 1000; //Зов Монолита
...
//        Log.e(TAG, "healByInfluence6 " + Thread.currentThread().getName() + "  " + newProps);
    ...
    public void calculateAccumulated(long delta)
    {
        ...
        {
            //При накоплении до 1 МЕ радиации жизнь восстанавливается самопроизвольно, накопленная радиация постепенно самостоятельно разрешается после достижения 100% жизни (1 SV в минуту).
...
//            }
        ...
        {
            //Когда накоплено более 1 SV излучения, жизнь начинает постепенно уменьшаться со скоростью, соответствующей накопленной радиации
    ...
    private boolean hit(int infl, long delta)
    {
        ...
        switch (infl)
        {
            case Influence.BURER:
                ...
                {
                    accumulatedBurer = 0;
                    newProps.subtractHealth(25); //Бюрер
                    if(newProps.getHealth() <= 0)
                    {
            ...
            case Influence.CONTROLLER:
                ...
                if(accumulatedController >= CONTROLLER_COOLDOWN)
                {
                    ...
                    newProps.subtractHealth(30); //Контролёр
                    if(newProps.getHealth() <= 0)
                    {
            ...
            case Influence.MONOLITH:
                ...
                if(accumulatedMonolith >= MONOLITH_COOLDOWN)
                {
                    ...
                    newProps.subtractHealth(25); //Зов Монолита
                    if(newProps.getHealth() <= 0)
                    {
                        newProps.setState(Player.STATE.W_MENTALLED);
                    }
            ...
            case Influence.EMISSION:
                ...
                if(accumulatedEmission >= EMISSION_COOLDOWN)
                {
                    ...
                    newProps.subtractHealth(20); //Выброс
                    if(newProps.getHealth() <= 0)
                    {
    ...
    public void calculateEnvDamage(long delta)
    {
        ...
        {
            net.afterday.compas.logging.Logger.d(inflPack.toString());
            return;
        }
